...
<?php 
	...
	$user_query = mysqli_query($conn, "SELECT * FROM user WHERE school_number = '$school_number' ");
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
					...
					<?php
						$sql = mysqli_query($conn, "SELECT * FROM user WHERE school_number = '$school_number' ");
						...
						<div class="table-responsive">
							...
							<?php
								if (isset($_POST['return'])) {
									...
									$update_copies = mysqli_query ($conn, "SELECT * from book where book_id = '$book_id' ") or die (mysqli_error());
									...
									mysqli_query($conn, "UPDATE book SET book_copies = '$new_book_copies' where book_id = '$book_id'") or die (mysqli_error());
									mysqli_query($conn, "UPDATE book SET remarks = '$remark' where book_id = '$book_id' ") or die (mysqli_error());
									...
									mysqli_query ($conn, "UPDATE borrow_book SET borrowed_status = 'returned', date_returned = '$date_returned_now', book_penalty = '$penalty' WHERE borrow_book_id= '$borrow_book_id' and user_id = '$user_id' and book_id = '$book_id' ") or die (mysqli_error());
									
									mysqli_query ($conn, "INSERT INTO return_book (user_id, book_id, date_borrowed, due_date, date_returned, book_penalty)
									values ('$user_id', '$book_id', '$date_borrowed', '$due_date', NOW(), '$penalty')") or die (mysqli_error());
									
									$report_history1=mysqli_query($conn, "select * from admin where admin_id = $id_session ") or die (mysqli_error());
						...
						<table class="table table-bordered">
							...
							<?php 
								if (isset($_POST['barcode'])){
									...
									$book_query = mysqli_query($conn, "SELECT * FROM book WHERE book_barcode = '$barcode' ") or die (mysqli_error($conn));
							...
							<?php 
								if (isset($_POST['borrow'])){
									...
									$trapBookCount= mysqli_query ($conn, "SELECT count(*) as books_allowed from borrow_book where user_id = '$user_id' and borrowed_status = 'borrowed'") or die (mysqli_error($conn));
									
									$countBorrowed = mysqli_fetch_assoc($trapBookCount);
									
									$bookCountQuery= mysqli_query ($conn, "SELECT count(*) as book_count from borrow_book where user_id = '$user_id' and borrowed_status = 'borrowed' and book_id = $book_id") or die (mysql_error());
									
									$bookCount = mysqli_fetch_assoc($bookCountQuery);
									
									$allowed_book_query= mysqli_query($conn, "select * from allowed_book order by allowed_book_id DESC ") or die (mysql_error());
									...
									mysqli_query($conn, "UPDATE book SET remarks = '$remark' where book_id = '$book_id' ") or die (mysqli_error($conn));
									
									mysqli_query($conn, "INSERT INTO borrow_book(user_id,book_id,date_borrowed,due_date,borrowed_status)
									VALUES('$user_id','$book_id','$date_borrowed','$due_date','borrowed')") or die (mysqli_error($conn));
									
									$report_history=mysqli_query($conn, "select * from admin where admin_id = $id_session ") or die (mysqli_error($conn));
									...
									mysqli_query($conn, "INSERT INTO report 
									(book_id, user_id, admin_name, detail_action, date_transaction)
									VALUES ('$book_id','$user_id','$admin_row','Borrowed Book',NOW())") or die(mysqli_error($conn));
