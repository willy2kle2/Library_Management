<?php include ('header.php'); ?>
	...
	<div class="col-xs-3">
		...
		<button type="submit" class="btn btn-primary btn-outline" style="margin:-34px -195px 0px 0px; float:right;" name="ok"><i class="fa fa-calendar-o"></i> Sort By Date Borrowed</button>
					...
					<?php 
					$count = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT(*) as total FROM `borrow_book`")) or die(mysqli_error($conn));
					$count1 = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT(*) as total FROM `borrow_book` WHERE `borrowed_status` = 'borrowed'")) or die(mysql_error());
					$count2 = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT(*) as total FROM `borrow_book` WHERE `borrowed_status` = 'returned'")) or die(mysqli_error());
					?>
						...
						<div class="table-responsive">
							...
							<?php
								$borrow_query = mysqli_query($conn, "SELECT * FROM borrow_book
									...
									ORDER BY borrow_book.borrow_book_id DESC") or die(mysqli_error($conn));
